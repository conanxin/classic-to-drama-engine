...
if (hasDist) {
  ...
  for (const prototype of graphicPrototypes) {
    ...
    const readable = html.replaceAll('&#39;', "'").replaceAll('&quot;', '"').replaceAll('&amp;', '&').replaceAll('&lt;', '<').replaceAll('&gt;', '>');
    const boundedEp01Repair = prototype.episode === 'EP01' && (readable.includes('data-comic-repair="P8R1"') || readable.includes('data-comic-repair="P8R2"'));
    const hookProbe = boundedEp01Repair ? '门锁好。别替我告别。' : prototype.end_hook;
